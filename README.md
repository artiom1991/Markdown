...
![The Markdown Guide](images/img.png)

> **Важно знать**, что `Markdown` генерирует `HTML-разметку` со своими встроенными стилями, поэтому в
> файлах `.md` допускается использование `HTML-разметки` с `inline-стилями`.

> __Где используется__? Чаще всего файлы с расширением `.md` используются для написания документации для программного кода,
> однако это не единственное применение формата. `Markdown` также используется для генерации
...
| `__*`     | Выделение жирным и наклонным шрифтом. **Пример**: `__*Bold&Italic*__` . **Аналог HTML**: `<strong><em>Bold&Italic</em></strong>` |
| `**_`     | Выделение жирным и наклонным шрифтом. **Пример**: `**_Bold&Italic_**` . **Аналог HTML**: `<strong><em>Bold&Italic</em></strong>` |
